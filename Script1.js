    ...
    <style>
      ...
      *{box-sizing:border-box}
      ...
      table{
        ...
        min-width:1000px; box-shadow:0 10px 30px rgba(0,0,0,.25);
      }
    ...
    <div class="wrap">
      ...
      <form method="post" action="${editItem ? `/update/${editItem.id}` : "/add"}" autocomplete="off">
        <div class="fields">
          ...
          <!-- Пустые поля оставлены по просьбе -->
          <div class="field"></div>
          <div class="field"></div>
        </div>

        <div class="note">Wymagane: <strong>name</strong>, <strong>sku</strong>, <strong>price</strong>, <strong>size</strong>, <strong>quantity</strong>.</div>
      ...
      <footer>UI — ciemny motyw, responsywne, lekko „admin panel”.</footer>
    </div>
  </body>
  </html>`;
...
app.post("/add", (req, res) => {
    ...
    try {
      ...
      VALUES (?, ?, ?, ?, ?, ?)
    `).run(safeStr(name), safeSku(sku), pr, safeSize(size), safeStr(color), qty);
    } catch (e) {
...
app.post("/update/:id", (req, res) => {
    ...
    try {
      ...
      UPDATE products
       ...
       WHERE id = ?
    `).run(safeStr(name), safeSku(sku), pr, safeSize(size), safeStr(color), qty, id);
    } catch (e) {
        if (String(e.message || "").includes("UNIQUE constraint failed: products.sku")) {
            return res.status(400).send("SKU musi być unikalne.");
        }
